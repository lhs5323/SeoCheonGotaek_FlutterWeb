...
class AboutPersonDetail extends StatelessWidget {
  const AboutPersonDetail({
    Key key,
  ...
  Widget build(BuildContext context) {
    ...
    Container(
        margin: EdgeInsets.only(top: widget.screenwidth * 0.028, right: widget.screenwidth * 0.02, left: widget.screenwidth * 0.02),
        ...
        child: Column(
            ...
            children: [
              Expanded(
                flex: 1,
                child: AutoSizeText("본관은 한산(韓山), 호는 청암(靑菴)이다. 고려 말기의 문인·학자 이색(李穡)의 후손으로 충청남도 서천군 기산면"
                  ...
                  children: [
                    Stack(alignment: Alignment.center, children: [
                      Image.asset(
                        ...
                        height: widget.screenheight * 0.6 * 0.631,
                      ),
                      SvgPicture.asset(
                        'assets/images/content_link_edge.svg',
                        width: widget.screenwidth * 0.6,
                        height: widget.screenheight * 0.6 * 0.6651,
                      ),

                    ],
                    ),
                    AutoSizeText("▲ 와세다  졸업기념 교장 선생님과 함께(맨 왼쪽이 이하복 선생)")
                  ],
                ),
              )
            ]))
